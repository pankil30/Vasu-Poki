...
function buildTileSizes(count: number) {
    ...
    // Ensure first tile is small and last N tiles are small
    const LAST_SMALL_COUNT = 11

    // Shuffle sizes
    const shuffled = seededShuffle(sizes, count * 97)

    // Force first tile to size 1
    shuffled[0] = 1

    // Force last LAST_SMALL_COUNT tiles to size 1
...
export default function Home() {
    ...
    const [apiDataDump, setApiDataDump] = useState<string | null>(null)
    ...
    useEffect(() => {
        const loadGames = async () => {
            try {
                const url = `${window.location.origin}/api/games`
                const response = await fetch(url, { cache: 'no-store' })
                console.log('[games] api response status', response.status)
                setApiStatus(response.status)
                const data = await response.json()
                console.log('[games] api response data', data)
                setApiDataDump(JSON.stringify(data, null, 2))

                // Support multiple possible shapes from the API route
    ...
    return (
        ...
        >
            <div className="relative h-full overflow-auto">
                ...
                >
                    {tileOrder.map((size, i) => {
                        ...
                        return (
                            ...
                            >
                                {game ? (
                                    <Link href={game.slug ? `/games/${game.slug}` : `/games/${game.id}`}>
                                        <div
                                            className="group relative block w-full h-full min-h-0 min-w-0 overflow-hidden rounded-lg shadow bg-black/10"
                ...
                {/* Categories section (POKi-like grid) */}
                <div >
                ...
                {/* <div className="mx-auto my-8 hidden w-[960px] h-[540px] overflow-hidden rounded-xl border border-black/10 bg-yellow-200 shadow-lg lg:block">
                    <iframe
                        src="https://www.twoplayergames.org/gameframe/granny-4"
                        title="Granny 4 game frame"
